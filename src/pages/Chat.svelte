<script lang="ts">
  ...
  import ChatElement from "../components/Chat/ChatElement.svelte";
  import { messages_store, last_visited_channel_store } from "../utils/stores";
  import { onDestroy, onMount } from "svelte";
  import { useLocation } from "svelte-routing";
  import type { Unsubscriber } from "svelte/store";
  import { afterUpdate } from "svelte";
  import { updateServersState } from "../utils/fetch";
  ...
  async function fetchMessagesOfChannel() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/messages/${currentChannel}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
